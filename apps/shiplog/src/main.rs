...
fn make_github_ingestor(user: &str, since: NaiveDate, until: NaiveDate, mode: &str, include_reviews: bool, no_details: bool, throttle_ms: u64, token: Option<String>, api_base: &str) -> GithubIngestor {
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Command::Collect { source, out, zip, redact_key, regen } => {
            ...
            match source {
                Source::Github { user, since, until, mode, include_reviews, no_details, throttle_ms, token, api_base } => {
                    ...
                    let (outputs, ws_source) = engine.run(ingest, &user, &window_label, &run_dir, zip)?;
        ...
        Command::Render { out, run, user, window_label, redact_key, zip } => {
            let key = get_redact_key(redact_key);
            let engine = create_engine(&key);
            ...
            if !events_path.exists() {
                anyhow::bail!("No ledger.events.jsonl found in {:?}. Run `shiplog collect` first.", run_dir);
            }

            let ing = JsonIngestor { events_path, coverage_path };
            let ingest = ing.ingest()?;
...
fn print_outputs(outputs: &shiplog_engine::RunOutputs, ws_source: WorkstreamSource) {
    println!("- {} ({})", outputs.packet_md.display(), 
        match ws_source {
            ...
            WorkstreamSource::Suggested => "using suggested workstreams (edit and rename to workstreams.yaml)",
