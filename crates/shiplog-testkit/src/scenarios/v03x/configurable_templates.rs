...
pub fn template_custom() -> Scenario {
    Scenario::new("User defines custom packet template")
        .given("a user has a custom Jinja2 template file at templates/custom.md", |ctx| {
            ctx.paths
                .insert("template_path".to_string(), "templates/custom.md".into());
            ctx.strings
                .insert("template_content".to_string(), "# Custom Packet\n{% for ws in workstreams %}...{% endfor %}".to_string());
        })
        ...
        .then("the packet should be rendered using the custom template", |ctx| {
            assert_true(
...
pub fn template_variables() -> Scenario {
    Scenario::new("User template includes custom variables")
        .given("a user has a custom template with variables like {{ user_name }}, {{ company }}", |ctx| {
            ctx.strings.insert("user_name".to_string(), "Alice".to_string());
            ctx.strings.insert("company".to_string(), "Acme Corp".to_string());
        })
        .given("they have configured these variables in their config", |ctx| {
        ...
        .when(
            ...
            |ctx| {
                ctx.flags.insert("packet_rendered".to_string(), true);
                ctx.strings
                    .insert("output_content".to_string(), "# Packet by Alice at Acme Corp".to_string());
                Ok(())
            },
        )
...
pub fn template_conditionals() -> Scenario {
    Scenario::new("User template includes conditional sections")
        .given("a user has a custom template with conditional sections", |ctx| {
            ctx.strings
                .insert("template_content".to_string(), "{% if show_details %}Details{% endif %}".to_string());
        })
        .given("the template shows a section only if {{ show_details }} is true", |ctx| {
        ...
        .when(
            ...
            |ctx| {
                ctx.flags.insert("packet_rendered".to_string(), true);
                ctx.strings
                    .insert("output_content".to_string(), "# Packet\nDetails section".to_string());
                Ok(())
            },
        )
...
pub fn template_loops() -> Scenario {
    Scenario::new("User template includes loops over workstreams")
        .given("a user has a custom template with {% for ws in workstreams %}", |ctx| {
            ctx.strings
                .insert("template_content".to_string(), "{% for ws in workstreams %}{{ ws.title }}\n{% endfor %}".to_string());
        })
        ...
        .when(
            ...
            |ctx| {
                ctx.flags.insert("packet_rendered".to_string(), true);
                ctx.strings
                    .insert("output_content".to_string(), "Workstream 1\nWorkstream 2\nWorkstream 3\nWorkstream 4\nWorkstream 5".to_string());
                Ok(())
            },
        )
        ...
        .then("each workstream should be rendered according to the template", |ctx| {
            let output = ctx.string("output_content").unwrap();
...
pub fn template_not_found() -> Scenario {
    Scenario::new("Template file does not exist")
        ...
        .then("the error should indicate the template file was not found", |ctx| {
...
pub fn template_syntax_error() -> Scenario {
    Scenario::new("Template has syntax errors")
        .given("a user has a template with Jinja2 syntax errors", |ctx| {
            ctx.strings
                .insert("template_content".to_string(), "{% if unclosed %}".to_string());
            ctx.flags.insert("has_syntax_error".to_string(), true);
        })
        .when(
            "they run \"shiplog render --template broken.md\"",
            |ctx| {
                ctx.flags.insert("command_failed".to_string(), true);
                ctx.strings.insert(
                    "error_message".to_string(),
        ...
        .then("the error should indicate the syntax error location", |ctx| {
            let error = ctx.string("error_message").unwrap();
...
pub fn template_undefined_variable() -> Scenario {
    Scenario::new("Template references undefined variable")
        .given("a user has a template that references {{ undefined_var }}", |ctx| {
            ctx.strings
                .insert("template_content".to_string(), "{{ undefined_var }}".to_string());
        })
        .when(
...
pub fn template_multi_source() -> Scenario {
    Scenario::new("Custom template works with all event sources")
        ...
        .when(
            "they run \"shiplog render --template custom.md\"",
            |ctx| {
                ctx.flags.insert("packet_rendered".to_string(), true);
        ...
        .then("the template should correctly render events from all sources", |ctx| {
            assert_true(
                ctx.flag("packet_rendered").unwrap_or(false),
                "packet rendered",
            )
...
pub fn template_redaction() -> Scenario {
    Scenario::new("Custom template preserves redaction")
        .given("a user has collected events with sensitive information", |ctx| {
            ctx.strings.insert("sensitive_title".to_string(), "Secret Project".to_string());
        })
        .given("they are rendering with a redaction profile", |ctx| {
            ctx.strings.insert("redaction_profile".to_string(), "public".to_string());
        })
        .given("they have a custom template", |ctx| {
            ctx.flags.insert("template_valid".to_string(), true);
        })
        ...
        .then("the rendered packet should not contain sensitive data", |ctx| {
            let output = ctx.string("output_content").unwrap();
...
pub fn template_large() -> Scenario {
    Scenario::new("Complex template with many workstreams")
        ...
        .given("they have a complex custom template", |ctx| {
            ctx.strings
                .insert("template_content".to_string(), "{% for ws in workstreams %}...{% endfor %}".to_string());
        })
        .when(
            "they run \"shiplog render --template complex.md\"",
            |ctx| {
                ctx.flags.insert("packet_rendered".to_string(), true);
                ctx.strings
        ...
        .then("rendering should complete within reasonable time (< 10 seconds)", |ctx| {
