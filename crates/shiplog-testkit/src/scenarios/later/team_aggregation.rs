...
pub fn team_aggregate_summary() -> Scenario {
    Scenario::new("User generates team-level shipping summary")
        .given("a user is a team lead", |ctx| {
            ctx.strings.insert("user_role".to_string(), "team_lead".to_string());
        })
        .given("they have access to multiple team members' shiplog ledgers", |ctx| {
        ...
        .when(
            ...
            |ctx| {
                ...
                ctx.flags.insert("member_sections_included".to_string(), true);
        ...
        .then("the packet should include sections for each team member", |ctx| {
            assert_true(
...
pub fn team_aggregate_aliases() -> Scenario {
    Scenario::new("User aggregates team with member aliases")
        .given("a user has team members with different display names", |ctx| {
            ctx.strings.insert("member_real_name".to_string(), "alice.smith".to_string());
            ctx.strings.insert("member_alias".to_string(), "Alice S.".to_string());
        })
        .given("they configure member aliases in a config file", |ctx| {
            ctx.strings.insert("config_file".to_string(), "team.yaml".to_string());
...
pub fn team_aggregate_missing_ledger() -> Scenario {
    Scenario::new("Member ledger not found")
        .given("a user specifies a team member", |ctx| {
            ctx.strings.insert("member_name".to_string(), "alice".to_string());
        })
        ...
        .then("the packet should be generated for available members", |ctx| {
...
pub fn team_aggregate_incompatible_version() -> Scenario {
    Scenario::new("Member ledger has incompatible version")
        .given("a user specifies a team member", |ctx| {
            ctx.strings.insert("member_name".to_string(), "bob".to_string());
        })
        .given("that member's ledger uses an incompatible schema version", |ctx| {
            ctx.strings
        ...
        .when(
            ...
            |ctx| {
                ...
                ctx.strings.insert(
                    "warning_message".to_string(),
                    "Warning: Ledger for 'bob' has incompatible version 0.1.0 (required: 0.2.0)".to_string(),
                );
...
pub fn team_aggregate_large() -> Scenario {
    Scenario::new("Team aggregation with many members")
        ...
        .when(
            "they run \"shiplog team-aggregate --members all\"",
            |ctx| {
                ctx.flags.insert("team_packet_generated".to_string(), true);
                ctx.strings
        ...
        .then("aggregation should complete within reasonable time (< 30 seconds)", |ctx| {
