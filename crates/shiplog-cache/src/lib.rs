...
impl CacheKey {
    ...
    pub fn search(query: &str, page: u32, per_page: u32) -> String {
        format!("search:{}:page{}:per{}", Self::hash_query(query), page, per_page)
    }
...
mod tests {
    ...
    fn cache_key_builder() {
        ...
        assert_eq!(pr_key, "pr:details:https://api.github.com/repos/owner/repo/pulls/42");
        
        let reviews_key = CacheKey::pr_reviews("https://api.github.com/repos/owner/repo/pulls/42", 2);
        assert_eq!(reviews_key, "pr:reviews:https://api.github.com/repos/owner/repo/pulls/42:page2");
    }
