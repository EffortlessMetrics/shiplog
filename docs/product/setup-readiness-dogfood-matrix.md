...
| 4 | Malformed manual journal | The setup model marks malformed manual journals as blocked, and the product proof shows doctor catches the setup block before repair resumes (#402, #403, #408). Doctor JSON now proves agents see the malformed manual journal as read-first setup work and do not receive `journal add --from-repair` while setup is broken (#415). | PR #416 should pin the full `doctor -> intake -> repair plan` route in the user-facing handoff. | Manual journal repair is a setup fix first; evidence repair resumes after schema validity returns. |
