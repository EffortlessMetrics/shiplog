...
3. **PR #192**: cache-internals external-call-site audit. Verified by workspace grep that no caller outside `crates/shiplog-cache/src/sqlite.rs` reaches into `ApiCache::{conn, default_ttl, max_size_bytes}`; advanced ledger entry `cpf-0005` to `"accessor-ready"`; added a rustdoc citation on `ApiCache`. *Limitation discovered after merge*: `clippy::disallowed_fields` is path-scope-blind — it fires on every `self.conn` access, including inside the owning `impl`. So an activation that named `ApiCache::conn` would not be a one-line change against the existing code; it would require either per-method `#[expect]` annotations or an inner-struct split.
4. **This PR**: inner-struct refactor for cache-internals. Move `{conn, default_ttl, max_size_bytes}` onto a private `ApiCacheInner` struct that `ApiCache` wraps; update every internal method to thread through `self.inner.*`; preserve the public `ApiCache` API exactly. External code cannot name `ApiCacheInner`, so the lint can target `ApiCache::*` without flagging any internal access. Ledger entry `cpf-0005` advances from `"accessor-ready"` to `"clippy-ready"`.
5. **Next PR**: activate `clippy::disallowed_fields` for cache-internals only (`clippy.toml` lists the three `ApiCache` fields from `cpf-0005`; `[workspace.lints.clippy].disallowed_fields = "deny"`; the ledger entry in `policy/clippy-lints.toml` moves from `[[planned]]` to `[[active]]`; `cpf-0005.activation_status` advances to `"lint-active"`). The inner-struct split in step 4 means activation requires no `#[expect]` annotations and no `policy/clippy-exceptions.toml` entries.
6. **Subsequent PRs**: repeat the same shape (external-call-site audit → inner-struct refactor if needed → activation) per class for the remaining five (bundle-paths, policy-ledger-metadata, redaction-internals, trust-receipts, source-opaque-ids).
