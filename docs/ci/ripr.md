...
> **Current status:** The lane produces real advisory `ripr` evidence.
> Public README badge data is repo-scoped and generated by `cargo xtask badges`;
> PR evidence is diff-scoped and generated by `cargo xtask ripr-pr` and
> `cargo xtask ripr-review-comments`. These surfaces must not be mixed.
...
| Trigger | PR open / sync for Rust diffs; manual workflow dispatch |
| Runner | `ubuntu-latest` |
| Base LEM | 4 |
| Default PR? | yes |
| Blocking | no (advisory) |
| Outputs | `target/ripr/pr/`, `target/ripr/review/`, GitHub step summary, warning annotations from `comments[]` |
...
Repository policy keeps the legacy suppression ledger at [`policy/ripr-suppressions.toml`](../../policy/ripr-suppressions.toml). The installed `ripr` CLI reads `.ripr/suppressions.toml` when present; until that ledger is migrated, PR evidence is advisory and suppression review remains a human policy step.
